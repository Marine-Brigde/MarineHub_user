...
import { User, Phone, Save, Loader2, CheckCircle2, Camera } from "lucide-react"
...
export default function BoatyardProfilePage() {
    ...
    const loadProfile = async () => {
        try {
            setIsLoading(true)
            setError("")
            const [profileResponse, boatyardResponse] = await Promise.all([
                getProfileApi(),
                getBoatyardDetailApi()
            ])
            ...
            if (profileResponse.status === 200 && profileResponse.data) {
                ...
                // Lấy commissionFeePercent từ boatyard detail API
                const commissionFeePercent = boatyardResponse.status === 200 ? boatyardResponse.data?.commissionFeePercent || 0 : 0
                        ...
                        <div className="relative">
                            <Avatar className="h-32 w-32 border-4 border-primary/20">
                                ...
                                <AvatarFallback className="text-2xl">
                                    {profileData.fullName?.charAt(0)?.toUpperCase() || "X"}
                                </AvatarFallback>
                            </Avatar>
                            {isEditing && (
                                <label
